...
struct Multivector {
  ...
  using value_t = typename algebra::value_t;  ///< the field over which the
  /// algebra is defined (e.g. float
  /// or double)
  using space = typename algebra::types;  ///< @ingroup typesystem call
  ///<MultivectorType>::space::<MultivectorName>
  /// to declare another named Type within the same
  /// algebra

  template <class B>
  using MultivectorB =
      Multivector<algebra, B>;  ///< another Type within same algebra
  ...
  /// the Euclidan subspace Dual Type (product of this and
  ...
  constexpr int get_num_bases() const { return Num; }
  ...
  auto operator*(const MultivectorB<B>& b) const
      -> decltype(algebra::gp(*this, b)) {
    return algebra::gp(*this, b);
  }

  template <class B>
  auto geometric_product(const MultivectorB<B>& b) const
      -> decltype(algebra::gp(*this, b)) {
    return algebra::gp(*this, b);
  }
  ...
  // Division \\(A/B\\)
  template <class B>
  auto operator/(const MultivectorB<B>& b) const RETURNS((*this * !b));

  /*-----------------------------------------------------------------------------
   *  Transformations (note mostly here for conformal metric)
   *  for instance null() only works in conformal metric (returns identity
   *in others)
   *  these are all defined in vsr_generic_op.h or in vsr_cga3D_op.h
   *-----------------------------------------------------------------------------*/

  /// Conformal Mapping \\(\\boldsymbol(x)\\to n_o + \\boldsymbol{x} +
  /// \\boldsymbol{x}^2n_\\infty \\)
...
template <typename algebra>
using GAEucPss = Multivector<algebra, typename algebra::types::euc>;

template <typename algebra>
struct GAE {
  template <bits::type... NN>
  using e = typename algebra::types::template e<NN...>;  // Multivector<algebra,
                                                         // typename
  // algebra::types::e_basis::template e<NN...> >;
};
...
struct NE {
  template <bits::type... NN>
  using e = typename GAE<euclidean<bits::dimOf(bits::blade((1 << (NN - 1))...)),
                                   T>>::template e<NN...>;
};
//
