...
struct Basis<X, XS...> {
  ...
  static void print() {
    printf("%s\t%s\t%d\n", bits::estring(X).c_str(),
           bits::bitString<6>(X).c_str(), X);
    TAIL::print();
  }
};
...
struct May {
  template <class A, class B>
  static constexpr A Be(const A& a, const B& b) {
    return a;
  }
};
template <>
struct May<false> {
  template <class A, class B>
  static constexpr B Be(const A& a, const B& b) {
    return b;
  }
};
...
struct Take {
  using Type = typename Cat<Basis<B::HEAD>,
                            typename Take<S - 1, typename B::TAIL>::Type>::Type;
};
...
struct InsertIdxOfEnd<true> {
  template <int A, class B>
  static constexpr int Call(int c) {
    return A == B::HEAD ? -1 : bits::lessThan(A, B::HEAD) ? c : c + 1;
  }
};
...
struct InsertIdxOfExists<false> {
  template <int A, class B>
  static constexpr int Call(int c) {
    return InsertIdxOfImpl<bits::lessThan(
        A, B::HEAD)>::  // Is in Correct Position?
...
struct InsertIdxOf {
  static constexpr int Call() {
    return InsertIdxOfImpl<bits::lessThan(A, B::HEAD)>::template Call<A, B>(0);
  }
};

template <int A>
struct InsertIdxOf<A, Basis<>> {
  static constexpr int Call(int c = 0) { return c; }
};
...
struct InsertImpl<false> {
  ...
  using Result = typename Cat<
      ...
      typename Cat<Basis<A>, typename Remove<S, B>::Type>::Type>::Type;
};
...
struct NotType {
  ...
  typedef typename Cat<typename Maybe<IS == -1, Basis<B::HEAD>, Basis<>>::Type,
                       typename NotType<A, typename B::TAIL>::Type>::Type Type;
};
template <class A>
...
struct MSign {
  static constexpr int Val =
      (AB & 1) ? MSign<typename M::TAIL, (AB >> 1), (SF * (M::HEAD))>::Val
               : MSign<typename M::TAIL, (AB >> 1), SF>::Val;
};
...
struct Exists {
  static constexpr bool Call() {
    return M::HEAD == N ? true : Exists<N, typename M::TAIL>::Call();
  }
};
...
struct metric {
  using type =
      typename Cat<typename RPlus<P>::Type,
                   typename RMinus<Q>::Type>::Type;  // metric as list of type T
  ...
  static const bool is_conformal =
      bConformal;  // whether to project metric stereographically
};
...
struct XList {
  ...
  static constexpr double Exec(const A& a, const B& b) {
    return 0;
  }
...
struct XList<X, XS...> {
  ...
  template <class A, class B>
  static constexpr auto Exec(const A& a, const B& b) ->
      typename A::algebra::value_t {
...
struct FindAllImpl<true, A> {
  ...
  using Result = typename FindAllImpl<A::HEAD::Res == N, typename A::TAIL>::
      template Result<N, XList<typename A::HEAD>,
                      typename XCat<Add, Tally>::Type>;
};
template <class A>
...
struct Index {
  ...
  typedef typename XCat<XList<One>,
                        typename Index<I, typename R::TAIL>::Type>::Type Type;
};
template <class I>
...
/*-----------------------------------------------------------------------------
 *  Take A List of Instructions, Reduce it to Get a Return Type, and Index That
...
struct Reverse {
  typedef typename XCat<XList<InstFlip<bits::reverse(A::HEAD), IDX>>,
                        typename Reverse<typename A::TAIL, IDX + 1>::Type>::Type
      Type;
};
template <int IDX>
...
struct Conjugate {
  typedef
      typename XCat<XList<InstFlip<bits::conjugate(A::HEAD), IDX>>,
                    typename Conjugate<typename A::TAIL, IDX + 1>::Type>::Type
          Type;
};
template <int IDX>
...
struct Involute {
  typedef
      typename XCat<XList<InstFlip<bits::involute(A::HEAD), IDX>>,
                    typename Involute<typename A::TAIL, IDX + 1>::Type>::Type
          Type;
};
template <int IDX>
...
constexpr int find(int n, const Basis<>,
                   int idx) {  // should not have default arg
  return -1;
}
template <class A>
...
template <class A, class B>
struct Cast {
  typedef typename XCat<XList<InstCast<find(A::HEAD, B())>>,
                        typename Cast<typename A::TAIL, B>::Type>::Type Type;
};
template <class B>
struct Cast<Basis<>, B> {
  typedef XList<> Type;
...
template <bits::type DIM>
using epss = Basis<bits::pss(DIM - 2)>;
template <bits::type DIM>
using origin = Basis<bits::origin<DIM>()>;
template <bits::type DIM>
using infinity = Basis<bits::infinity<DIM>()>;
template <bits::type DIM>
using eplane = Basis<bits::eplane<DIM>()>;
template <bits::type DIM>
...
struct VPrint {
  ...
  static void Call(B& m) {
    for (int i = 0; i < B::Num; ++i) std::cout << m[i] << "\t";
    std::cout << std::endl;
  }
};
