...
struct Basis<X, XS...>
{
  ...
  static void print()
  {
    printf("%s\t%s\t%d\n", bits::estring(X).c_str(), bits::bitString<6>(X).c_str(), X);
    TAIL::print();
  }
};
...
struct InsertIdxOf
{
  static constexpr int Call() { return InsertIdxOfImpl<bits::lessThan(A, B::HEAD)>::template Call<A, B>(0); }
};
...
struct InsertImpl<false>
{
  ...
  using Result =
      typename Cat<typename Take<S, B>::Type, typename Cat<Basis<A>, typename Remove<S, B>::Type>::Type>::Type;
};
...
struct MSign
{
  static constexpr int Val = (AB & 1) ? MSign<typename M::TAIL, (AB >> 1), (SF * (M::HEAD))>::Val
...
struct Blade1
{
  static constexpr auto Vec()
      RETURNS(cat(Basis<bit<(TOT - DIM)>()>(), Blade1<TOT, DIM - 1>::Vec())) typedef decltype(Vec()) VEC;
};
...
struct FindAllImpl<true, A>
{
  template <int N, class Add, class Tally>
  using Result =
      typename FindAllImpl<A::HEAD::Res == N, typename A::TAIL>::template Result<N, XList<typename A::HEAD>,
...
struct FindAllImpl<false, A>
{
  template <int N, class Add, class Tally>
  using Result =
      typename FindAllImpl<A::HEAD::Res == N, typename A::TAIL>::template Result<N, XList<typename A::HEAD>, Tally>;
};
template <int N, class A>
struct FindAll
{
  using Type =
      typename FindAllImpl<A::HEAD::Res == N, typename A::TAIL>::template Result<N, XList<typename A::HEAD>, XList<>>;
};
template <int N>
...
struct Reverse
{
  typedef typename XCat<XList<InstFlip<bits::reverse(A::HEAD), IDX>>,
                        typename Reverse<typename A::TAIL, IDX + 1>::Type>::Type Type;
};
template <int IDX>
...
struct Conjugate
{
  typedef typename XCat<XList<InstFlip<bits::conjugate(A::HEAD), IDX>>,
                        typename Conjugate<typename A::TAIL, IDX + 1>::Type>::Type Type;
};
template <int IDX>
...
struct Involute
{
  typedef typename XCat<XList<InstFlip<bits::involute(A::HEAD), IDX>>,
                        typename Involute<typename A::TAIL, IDX + 1>::Type>::Type Type;
};
template <int IDX>
