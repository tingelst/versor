...
#include <vsr/detail/vsr_xlists.h>    ///<-- list processing functions
#include <vsr/detail/vsr_products.h>  ///<-- compile time processing of instructions ("arrows" or "morphisms")
...
template <class algebra, bool, bool>
struct algebra_impl;
...
struct algebra
{
  ...
  template <class B>
  static mv_t<typename ICat<typename NotType<Basis<0>, B>::Type, Basis<0>>::Type> sumv(VSR_PRECISION a,
                                                                                       const mv_t<B>& b)
  {
