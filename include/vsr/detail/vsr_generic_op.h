...
struct Proj
{
  ...
  static auto Call(VSR_PRECISION dist, const TVec& v)
      RETURNS((Proj<DIM - 1>::Call(dist, v.template cast<OneDown>() * (dist / (dist - v[DIM - 1])))));
...
struct Gen
{
  ...
  */
   ...
   */
  template <class A, class B>
  static auto trv(const Multivector<A, B>& a) RETURNS(a.template copy<GATnv<A>>() + 1);
  ...
  */
  ...
  /*! Generate a Dilation from a point p by amt t
      ...
      @param Amt t -- to pass in a relative amt (i.e. t=.5 for half size or t=2 for VSR_PRECISION), pass in
     std::log(t)
  */
  ...
  */
  ...
  static auto ratio(const GAVec<A>& a, const GAVec<A>& b) -> decltype((a * b))
  {
    ...
    // 180 degree check
    if (a == b.conjugation())
    {  // fabs ((a<=b)[0]) > .999999) {//a == b.conjugation() ) {
      // printf("180!\n");
...
struct Round
{
  ...
  */
  ...
  {
    auto s = null(v);
    (r > 0) ? s.template get<bits::infinity<A::dim>()>() -= .5 *(r * r) : s.template get<bits::infinity<A::dim>()>() +=
                                                                          .5 * (r * r);
    return s;
  }
  ...
  */
  ...
  static constexpr auto direction(const A& s)
      RETURNS(((typename A::space::infinity(-1) <= s) ^ typename A::space::infinity(1)));
  ...
  */
   ...
   * */
  template <class A>
  static constexpr auto carrier(const A& s) RETURNS(s ^ typename A::space::infinity(1));
  ...
  */
  ...
  static constexpr auto produce(const A& dls, const S& flat)
      RETURNS(dls ^ ((dls <= (flat.inv() * typename A::space::infinity(1))) * -1.0));

  /*!
    Creates a real / imaginary round from an imaginary / real round
   */
  template <class A>
  static constexpr auto real(const A& s) RETURNS(produce(Round::dls(Round::loc(s), -Round::rad(Round::sur(s))),
   ...
   */
  template <class A>
  static constexpr auto imag(const A& s) RETURNS(produce(Round::dls(Round::loc(s), Round::rad(Round::sur(s))),
                                                         typename A::space::origin(-1) <= Round::dir(s)));
  /*!
...
struct Flat
{
  /*! Direction of Direct Flat
    ...
    */
  template <class A, class B>
  static constexpr auto direction(const Multivector<A, B>& f) RETURNS(GAInf<A>(-1) <= f);
  /*! Direction of Direct Flat
        @param Direct Flat [ Plane (Pln) or Line (Lin) ]
        @returns \direction
    */
  ...
  */
  ...
  template <class A>
  static constexpr auto dlp(const GAPnt<A>& pnt, const GADrv<A>& drv) RETURNS(pnt <= drv);
...
struct Tangent
{
  ...
  */
  template <class A>
  static constexpr auto at(const A& r, const typename A::space::point& p) RETURNS(p <= r.inv());
