...
struct PushMink
{
  typedef XList<Blade<(a ^ bits::origin<dim>()) ^ bits::EP<dim>(), 2>,
                Blade<(a ^ bits::origin<dim>()) ^ bits::EM<dim>(), 2> > ORI;
  typedef XList<Blade<(a ^ bits::infinity<dim>()) ^ bits::EP<dim>(), -1>,
                Blade<(a ^ bits::infinity<dim>()) ^ bits::EM<dim>(), 1> > INF;
...
struct PopMink
{
  typedef XList<Blade<(A::BIT ^ bits::EP<dim>()) ^ bits::origin<dim>(), 1 * A::SIGN>,
                Blade<(A::BIT ^ bits::EP<dim>()) ^ bits::infinity<dim>(), -2 * A::SIGN> > ORI;
  typedef XList<Blade<(A::BIT ^ bits::EM<dim>()) ^ bits::origin<dim>(), 1 * A::SIGN>,
                Blade<(A::BIT ^ bits::EM<dim>()) ^ bits::infinity<dim>(), 2 * A::SIGN> > INF;
...
struct SplitGP
{
  typedef typename XCat<typename SubSplitGP<typename A::HEAD, B, M>::Type,
                        typename SplitGP<typename A::TAIL, B, M>::Type>::Type Type;
};
...
struct SubSplitOP
{
  static const int Met = MSign < M, A::BIT& B::HEAD::BIT, bits::signFlip(A::BIT, B::HEAD::BIT) ? -1 : 1 > ::Val;
  typedef Blade<A::BIT ^ B::HEAD::BIT, Met * A::SIGN * B::HEAD::SIGN> One;
  typedef typename XCat<typename Maybe<bits::outer(A::BIT, B::HEAD::BIT), XList<One>, XList<> >::Type,
                        typename SubSplitOP<A, typename B::TAIL, M>::Type>::Type Type;
};
template <class A, class M>
...
struct SplitOP
{
  typedef typename XCat<typename SubSplitOP<typename A::HEAD, B, M>::Type,
                        typename SplitOP<typename A::TAIL, B, M>::Type>::Type Type;
};
template <class B, class M>
...
struct SubSplitIP
{
  static const int Met = MSign < M, A::BIT& B::HEAD::BIT, bits::signFlip(A::BIT, B::HEAD::BIT) ? -1 : 1 > ::Val;
  typedef Blade<A::BIT ^ B::HEAD::BIT, Met * A::SIGN * B::HEAD::SIGN> One;
  typedef typename XCat<typename Maybe<bits::inner(A::BIT, B::HEAD::BIT), XList<One>, XList<> >::Type,
                        typename SubSplitIP<A, typename B::TAIL, M>::Type>::Type Type;
};
template <class A, class M>
...
struct SplitIP
{
  typedef typename XCat<typename SubSplitIP<typename A::HEAD, B, M>::Type,
                        typename SplitIP<typename A::TAIL, B, M>::Type>::Type Type;
};
template <class B, class M>
...
struct Pop
{
  typedef typename XCat<typename PopMink<typename A::HEAD, dim>::Type, typename Pop<typename A::TAIL, dim>::Type>::Type
      Type;
};
...
constexpr bits::type CalcSum(bits::type a, bits::type b) { return a + b; }
...
struct CompressIt<P, 2>
{
  ...
  typedef typename Maybe<compare(A(), B()), XList<typename Sum<A, B>::Type>, P>::Type Type;
};
...
struct CompressIt<P, 4>
{
  ...
  typedef typename Maybe<compare(B(), C()), XList<typename Sum<A, D>::Type, typename Sum<B, C>::Type>,
                         XList<typename Sum<A, C>::Type, typename Sum<B, D>::Type> >::Type Type;
};
...
{
  typedef typename XCat<typename Maybe<S::HEAD::SIGN == 0, XList<>, XList<typename S::HEAD> >::Type,
                        typename EliminateZeros<typename S::TAIL>::Type>::Type Type;
};
