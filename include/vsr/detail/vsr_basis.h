...
#ifndef VERSOR_VSR_DETAIL_BASIS_H_
#define VERSOR_VSR_DETAIL_BASIS_H_
...
constexpr type grade(type a, type c = 0) { return a > 0 ? (a & 1 ? grade(a >> 1, c + 1) : grade(a >> 1, c)) : c; }

/// Highest Dimension Represented by blade (finds leftmost bit)
constexpr type dimOf(type b, type dim = 0) { return (b > 0) ? dimOf(b >> 1, dim + 1) : dim; }
...
constexpr bool inner(type a, type b) { return !((grade(a) > grade(b)) || (grade(a ^ b) != (grade(b) - grade(a)))); }
...
constexpr int cpow(int x, int N) { return (N > 0) ? (x * cpow(x, N - 1)) : (1.0); }
...
constexpr bool reverse(type a) { return cpow(-1, (grade(a) * (grade(a) - 1) / 2.0)) == -1; }
...
constexpr bool conjugate(type a) { return cpow(-1, (grade(a) * (grade(a) + 1) / 2.0)) == -1; }
...
constexpr type pss(type dim) { return (dim == 0) ? 0 : (1 << (dim - 1)) | pss(dim - 1); }
...
#endif  // VERSOR_VSR_DETAIL_BASIS_H_
